...
from libraries.maimai_best_50 import computeRa, generateAchievementList
from libraries.maimaidx_api_data import maiApi
from libraries.maimaidx_error import *
from libraries.maimaidx_music import Music, mai
...
async def rise_score_data(qqid: int, username: Optional[str], rating: str, score: str,
    ...
    try:
        ...
        msg = image_to_base64(text_to_image(result.strip()))
        ...
        if len(song_remain_difficult) > 0:
            if len(song_remain_difficult) < 60:
                ...
                if len(song_remain_difficult) > 10:
                    msg = image_to_base64(text_to_image(msg.strip()))
            else:
...
async def level_achievement_list_data(qqid: int, username: Optional[str], rating: str, page: Optional[str],
    ...
    try:
        ...
        log.error(traceback.format_exc())
        msg = f'未知错误：{type(e)}\n请联系Bot管理员'
    return image_to_base64(text_to_image(msg.strip()))
...
async def rating_ranking_data(name: Optional[str], page: Optional[int]) -> str:
    ...
    try:
        ...
        if name:
            ...
            data = image_to_base64(text_to_image(msg.strip()))
    except Exception as e:
        log.error(traceback.format_exc())
        data = f'未知错误：{type(e)}\n请联系Bot管理员'
    return data
